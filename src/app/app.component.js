...
var three_dragcontrols_1 = require("three-dragcontrols");
var three_obj_loader_1 = require("three-obj-loader");
var three_mtl_loader_1 = require("three-mtl-loader");
...
var CDCApp = /** @class */ (function () {
    function CDCApp() {
        this.CDCApp = (function () {
            function CDCApp(_prodServ, _utils, http, __decorate) {
                ...
                this.controls = new three_dragcontrols_1.DragControls(this.camera, this.renderer.domElement);
                this.raycaster = new THREE.Raycaster();
                this.mouse = { clientX: 0, clientY: 0, layerX: 0, layerY: 0 };
                this.planeHelp = new THREE.Mesh(new THREE.PlaneBufferGeometry(500, 500, 8, 8), new THREE.MeshBasicMaterial({ color: 0xffffff }));
            ...
            CDCApp.prototype.ngOnInit = function () {
                ...
                this._prodServ.getStyles(this.catSelect).subscribe(function (st) {
            ...
            CDCApp.prototype.loadWall = function (name, filename, position, rotation) {
                ...
                var loader = new three_obj_loader_1.OBJLoader(manager);
                var promise = new Promise(function (resolve, reject) {
                    var mtlLoader = new three_mtl_loader_1.MTLLoader();
                    mtlLoader.setPath('../assets/models/wall/horizontal/');
                    mtlLoader.load(filename + '.mtl', function (materials) {
                        materials.preload();
                        var objLoader = new three_obj_loader_1.OBJLoader();
                ...
                var promise = new Promise(function (resolve, rejcet) {
                    ...
                    mtlLoader.load(filename + '.mtl', function (materials) {
                        ...
                        objLoader.load(filename + '.obj', function (object) {
                            ...
                            var scaleHeight = 1;
                            if (self.selectStyle.roofstyle === 'gable' &&
                            ...
                            if (self.selectStyle.$.rooftype === 'ab,1' ||
                                self.selectStyle.$.name === 'Barside') {
                                object.position.z = -(self.configSelect.depth * 12 -
                                    objSize.z / 2 +
                            ...
                            self.comparePercentage = Math.round((xhr.loaded / xhr.total) * 100);
                            console.log('Loading room: ' + self.comparePercentage + '% compvare');
                        }, function (error) {
                            console.log('There was an error');
                        });
                    });
            ...
            CDCApp.prototype.loadWallMask = function (position, objSize) {
                ...
                objLoader.load(filename, function (object) {
                    ...
                    object.scale.set(scaleWidth + 0.06, scaleHeight + 0.03, scaleDepth + 0.06);
            ...
            CDCApp.prototype.loadStyleBasicOptionsFromSide = function (side) {
                ...
                var options = [];
                var totalOptions;
                console.log('Selected side: ' + side + ' Total # of options: ' + totalOptions, self.configSelect['side' + side].option);
                    ...
                    if (totalOptions === 0) {
                        ...
                        var _loop_3 = function (i) {
                            ...
                            switch (element.itemClass) {
                                case 'window':
                                    self
                                        .loadWindowOption(side - 1, element.ordercode, element.hPosition)
                                        .then(function (loadedWindow) {
                                        var shutters = self.getElementAccessories(side, 'shutter', element.position);
                                        if (shutters) {
                                            self.loadNewBuildingWindowShutters(loadedWindow, shutters.ordercode);
                                        }
                                        var flowerbox = self.getElementAccessories(side, 'flowerbox', element.position);
                                        if (flowerbox) {
                                            self.loadNewBuildingWindowFlowerbox(loadedWindow, flowerbox.ordercode);
                                        }
                                    ...
                                    break;
                                case 'door':
                                    console.log('[loadStyleBasicOptionsFromSide] Loading ' + element.itemClass);
                                    self
                                        .loadDoorOption('wall-' + (side - 1), element.ordercode, element.hPosition)
                                    ...
                                    break;
                                default:
                                    console.log('[loadStyleBasicOptionsFromSide] Ignore this loading ' +
                                        element.itemClass);
                                    optionsProcessed++;
                                    if (optionsProcessed === totalOptions) {
                                        resolve(buildingOptions);
            ...
            CDCApp.prototype.getElementAccessories = function (sideNo, accType, position) {
                var self = this;
                for (var j = 1; j <= self.configSelect['side' + sideNo].option.length; j++) {
            ...
            CDCApp.prototype.loadNewBuildingWindowShutters = function (targetObject, ordercode) {
                var self = this;
                ...
                mtlLoader.load(ordercode.toLowerCase() + '_left.mtl', function (materials) {
                    ...
                    objLoader.setMaterials(materials);
                    objLoader.load(ordercode.toLowerCase() + '_left.obj', function (object) {
                ...
                mtlLoader.load(ordercode.toLowerCase() + '_right.mtl', function (materials) {
                    ...
                    objLoader.setPath('../assets/models/shutter/');
                    objLoader.setMaterials(materials);
                    objLoader.load(ordercode.toLowerCase() + '_right.obj', function (object) {
            ...
            CDCApp.prototype.loadNewBuildingWindowFlowerbox = function (targetObject, ordercode) {
                var self = this;
                var targetSize = self.objectSize(targetObject);
                var mtlLoader = new three_mtl_loader_1.MTLLoader();
                ...
                mtlLoader.load(ordercode.toLowerCase() + '.mtl', function (materials) {
                    ...
                    objLoader.load(ordercode.toLowerCase() + '.obj', function (object) {
            ...
            CDCApp.prototype.loadTerraceOption = function () {
                ...
                mtlLoader.load('barside.mtl', function (materials) {
                    ...
                    objLoader.load('barside.obj', function (object) {
                        var objSize = self.objectSize(object);
            ...
            CDCApp.prototype.loadGlenEchoAccessory = function () {
                ...
                mtlLoader.load('glen_echo_roof_acc.mtl', function (materials) {
                    ...
                    var objLoader = new three_obj_loader_1.OBJLoader();
                    objLoader.setPath('../assets/models/accessories/');
                    objLoader.load('glen_echo_roof_acc.obj', function (object) {
                        var objSize = self.objectSize(object);
            ...
            CDCApp.prototype.loadWindsorColumns = function () {
                ...
                mtlLoader.load('windsor_col.mtl', function (materials) {
                    ...
                    var objLoader = new three_obj_loader_1.OBJLoader();
                    objLoader.setPath('../assets/models/accessories/');
                    objLoader.load('windsor_col.obj', function (object) {
                        var objSize = self.objectSize(object);
            ...
            CDCApp.prototype.roofIsFlat = function () {
                var res = false;
                if (this.selectStyle.$.name === 'Bar Harbor' ||
            ...
            CDCApp.prototype.isWithinBoundsOf = function (object, target) {
                ...
                if (objectBox.min.x >= targetBox.min.x &&
            ...
            CDCApp.prototype.droppedOption = function ($event, dragData) {
                ...
                if (this.highlights.length !== 0) {
                    ...
                    for (var _i = 0, _a = this.highlights; _i < _a.length; _i++) {
                        var highlight = _a[_i];
                        var intersects = this.raycaster.intersectObjects(highlight.children, true);
                        console.log('This element intersects with: ', intersects);
                        if (intersects.length !== 0) {
                            placedCorrectly = true;
                            switch (self.draggedObject.opt_type) {
                                case 'flowerbox':
                                    this.loadFlowerboxOption(highlight, self.draggedObject.order_code);
                                    break;
                                case 'shutter':
            ...
            CDCApp.prototype.loadDroppedOption = function ($event, object, targetObject) {
                var self = this;
                var localMouse = new THREE.Vector2();
                localMouse.x =
                    ($event.layerX / this.renderer.domElement.clientWidth) * 2 - 1;
                ...
                this.raycaster.setFromCamera(localMouse, this.camera);
                var targetSize = self.objectSize(targetObject);
                var intersects = this.raycaster.intersectObjects(targetObject.children, true);
                var mtlLoader = new three_mtl_loader_1.MTLLoader();
                mtlLoader.setPath('../assets/models/' + self.draggedObject.opt_type + '/');
                mtlLoader.load(self.draggedObject.order_code.toLowerCase() + '.mtl', function (materials) {
                    materials.preload();
                    var objLoader = new three_obj_loader_1.OBJLoader();
                    objLoader.setPath('../assets/models/' + self.draggedObject.opt_type + '/');
                    objLoader.load(self.draggedObject.order_code.toLowerCase() + '.obj', function (object) {
                        object.name = name;
                        object.position.x = 0;
                        object.position.y = 0;
            ...
            CDCApp.prototype.createMaterialCedarElement = function () {
                var texture = new THREE.TextureLoader().load('../assets/textures/cedar-element.jpg');
            ...
            CDCApp.prototype.addCubeMap = function () {
                var skyGeometry = new three_1.BoxGeometry(1024, 1024, 1024);
                var directions = ['posx', 'negx', 'posy', 'negy', 'posz', 'negz'];
                var materialArray = [];
                for (var i = 0; i < 6; i++) {
                    materialArray.push(new THREE.MeshBasicMaterial({
                        map: THREE.ImageUtils.loadTexture('../assets/threeTextures/cube/' +
                            this.currentBG +
                            '/' +
                            directions[i] +
                            '.jpg'),
            ...
            CDCApp.prototype.changeSidingsTexture = function (objectName, newTexture) {
                ...
                var _loop_4 = function (i) {
                    ...
                    var texture = new THREE.TextureLoader().load('/assets/textures/' + textureName + '.jpg');
                ...
                var this_1 = this, texture, object;
            ...
            CDCApp.prototype.render = function () {
                ...
                (function render() {
                    ...
                    if ((self.cameraAngle > 115 && self.cameraAngle < 180) ||
                    ...
                    if ((self.cameraAngle > -25 && self.cameraAngle <= 0) ||
            ...
            CDCApp.prototype.loadTwoDormers = function () {
                ...
                mtlLoader.load('dormer_1.mtl', function (materials) {
                    ...
                    objLoader.setMaterials(materials);
                    objLoader.load('dormer_2.obj', function (object) {
                        object.name = 'dormer';
                        var objectSize = self.objectSize(object);
                    ...
                    objLoader.load('dormer_mask.obj', function (object) {
            ...
            CDCApp.prototype.loadWindowOption = function (side, order_code, position) {
                ...
                var promise = new Promise(function (resolve, reject) {
                    ...
                    mtlLoader.load(order_code.toLowerCase() + '.mtl', function (materials) {
                        ...
                        objLoader.setMaterials(materials);
                        objLoader.load(order_code.toLowerCase() + '.obj', function (object) {
                            object.name = 'window-' + position;
                            var pos = self.calculateWindowPosition(side, object, position, targetObject);
            ...
            CDCApp.prototype.calculateWindowPosition = function (side, object, position, targetObj) {
            ...
            CDCApp.prototype.loadFlowerboxOption = function (targetObject, order_code) {
                ...
                mtlLoader.load(order_code.toLowerCase() + '.mtl', function (materials) {
                    ...
                    objLoader.load(order_code.toLowerCase() + '.obj', function (object) {
                        ...
                        targetObject.add(object);
                        console.log('After loading the flowbox, this is the target object:  ', targetObject);
                        self.render();
                    }, function (xhr) {
                        self.comparePercentage = Math.round((xhr.loaded / xhr.total) * 100);
                    }, function (error) {
            ...
            CDCApp.prototype.loadRoofingOptions = function () {
                var self = this["this"].configSelect.general.option.forEach(function (option) {
                    if (option) {
                        if (option) {
                            if (option.itemClass === 'cupola') {
                                ...
                                mtlLoader.load(order_code_1.toLowerCase() + '.mtl', function (materials) {
                                    materials.preload();
                                    var objLoader = new three_obj_loader_1.OBJLoader();
                                    objLoader.setPath('/assets/models/roofing/');
                                    objLoader.setMaterials(materials);
                                    objLoader.load(order_code_1.toLowerCase() + '.obj', function (object) {
                                        var objectSize = self.objectSize(object);
                ...
                var promise = new Promise(function (resolve, reject) {
                    ...
                    mtlLoader.load(order_code.toLowerCase() + '.mtl', function (materials) {
                        ...
                        objLoader.load(order_code.toLowerCase() + '.obj', function (object) {
                            ...
                            var pos = self.calculateDoorPosition(object, position, targetObject);
                            object.position.x = pos.x;
                            object.position.y = pos.y;
                            object.position.z = pos.z;
                            var scaleHeight;
            ...
            CDCApp.prototype.calculateDoorPosition = function (object, position, targetObj) {
                var targetPos = targetObj.getWorldPosition();
                var targetSize = this.objectSize(targetObj);
                var objectSize = this.objectSize(object);
                var objectPos = {
            ...
            CDCApp.prototype.clearStage = function () {
                ...
                var promise = new Promise(function (resolve, reject) {
                    ...
                    var roofChildren = self.roof.children.filter(function (element) {
                        return (element.type !== 'DirectionalLight' &&
                            element.type !== 'AmbientLight');
                    });
                    roofChildren.forEach(function (ch) {
                        self.roof.remove(ch);
                    });
                    var roofOptionsChildren = self.roofOptions.children.filter(function (element) {
                        return (element.type !== 'DirectionalLight' &&
                            element.type !== 'AmbientLight');
                    });
            ...
            CDCApp.prototype.onProductStyleClicked = function (style) {
                ...
                this._prodServ.getProductConfiguration(style.$.defaultConfig).subscribe(function (res) {
                    _this.configSelect = res;
                    if (!Array.isArray(res.general.option)) {
                        _this.configSelect.general.option = [res.general.option];
                    }
            ...
            CDCApp.prototype.shareOnFacebook = function () {
                ...
                this.http
                    .post('https://api.cloudinary.com/v1_1/pervasive/image/upload', formData, { headers: headers })
                    .subscribe(function (data) {
                    var info = data.json();
                    window.open('http://www.facebook.com/sharer.php?u=' + info.url, 'Write your message here', 'width=500, height=500, scrollbars=yes, resizable=no');
                });
            };
            ...
            ], CDCApp.prototype, 'elementRef', void 0);
            CDCApp = __decorate([
                core_1.Component({
                    ...
                    animations: [
                        animations_1.trigger('stylesDrawertrigger', [
                            animations_1.state('inactive', animations_1.style({ transform: 'translateY(19vh)' })),
                            animations_1.state('active', animations_1.style({ transform: 'translateY(0)' })),
                        ...
                        animations_1.trigger('optionsDrawertrigger', [
                            animations_1.state('inactive', animations_1.style({ left: '-25%' })),
                            animations_1.state('active', animations_1.style({ left: '15%' })),
                            animations_1.transition('inactive => active', animations_1.animate('400ms ease-in')),
                            animations_1.transition('active => inactive', animations_1.animate('400ms ease-out'))
                        ]),
                        animations_1.trigger('sidingsDrawertrigger', [
                            animations_1.state('inactive', animations_1.style({ left: '-25%' })),
                            animations_1.state('active', animations_1.style({ left: '15%' })),
                            animations_1.transition('inactive => active', animations_1.animate('400ms ease-in')),
                            animations_1.transition('active => inactive', animations_1.animate('400ms ease-out'))
                        ]),
                        animations_1.trigger('trimDrawertrigger', [
                            animations_1.state('inactive', animations_1.style({ left: '-25%' })),
                            animations_1.state('active', animations_1.style({ left: '15%' })),
                            animations_1.transition('inactive => active', animations_1.animate('400ms ease-in')),
                            animations_1.transition('active => inactive', animations_1.animate('400ms ease-out'))
                ...
                __metadata('design:paramtypes', product_service_1.ProductService, utilities_service_1.UtilitiesService, http_1.Http)
            ], CDCApp);
            return CDCApp;
        })();
    }
    __decorate([
        core_1.ViewChild('rendererContainer', { static: false })
    ], CDCApp.prototype, "rendererContainer");
    return CDCApp;
}());
exports.CDCApp = CDCApp;
